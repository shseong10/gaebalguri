...
public class BoardFileManager {
    ...
    @Autowired
    private FileDao bDao;

    // private fullPath="c:/dskjfkdsjfkldsjflksdjfksdjfkls/upload"
    // 파일 업로드, DB저장
    public boolean fileUpload(List<MultipartFile> attachments, HttpSession session, String sb_num) {
        ...
        fMap.put("bf_sb_num", String.valueOf(sb_num));
        ...
        for (MultipartFile mf : attachments) {
            ...
            try {
                ...
                // Map버전
                result = bDao.boardfileInsertMap(fMap);
                if (result == false)
                    break;
            } catch (IOException e) {
    ...
    //	 파일 다운로드
    public ResponseEntity<Resource> fileDownload(ProfileFile bfile, HttpSession session) throws IOException {
        log.info("fileDownload()");
        // 파일 저장 경로
        String realpath = session.getServletContext().getRealPath("/");
        realpath += "upload/" + bfile.getPf_sysFileName();
        ...
        String fileName = URLEncoder.encode(bfile.getPf_oriFileName(), "UTF-8");
